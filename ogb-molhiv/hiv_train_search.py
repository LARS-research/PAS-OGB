...
parser.add_argument('--arch_learning_rate_min', type=float, default=0.0005, help='minimum learning rate for arch encoding')
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % 1 == 0:
            logging.info('epoch=%s, train_auc=%f, train_loss=%f, valid_auc=%f, valid_loss=%f, test_auc=%f, test_loss=%f, explore_num=%s', epoch, train_auc['rocauc'], train_obj, valid_acc['rocauc'], valid_obj, test_acc['rocauc'], test_obj, model.explore_num)
            print('epoch={}, train_auc={:.04f}, train_loss={:.04f},valid_auc={:.04f}, valid_loss={:.04f},test_auc={:.04f},test_loss={:.04f}, explore_num={}'.
                  format(epoch, train_auc['rocauc'], train_obj, valid_acc['rocauc'], valid_obj, test_acc['rocauc'], test_obj, model.explore_num))
...
def train_graph(data, model, criterion, model_optimizer, arch_optimizer, lr, arch_lr):
    ...
    return evaluator.eval(input_dict), total_loss
def infer_graph(data_, model, criterion, mode='none'):
    ...
    test_input_dict = {'y_true': y_test_true, 'y_pred': y_test_pred}

    return valid_loss, evaluator.eval(val_input_dict), \
           test_loss, evaluator.eval(test_input_dict)
