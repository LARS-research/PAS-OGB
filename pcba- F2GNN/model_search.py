...
class NaOp(nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr):
        # if self.with_linear:
        #     return self.op(x, edge_index, edge_attr = edge_attr) + self.linear(x)
        # else:
        return self.op(x, edge_index, edge_attr = edge_attr)
    ...
    def forward(self, x, edge_index, edge_attr, weights):
        ...
        for w, op in zip(weights, self._ops):
            if self.with_linear:
                ...
                mixed_res.append(w * F.elu(op(x, edge_index, edge_attr = edge_attr) + linear_x))
            else:
                mixed_res.append(w * F.elu(op(x, edge_index, edge_attr = edge_attr)))
...
class Network(nn.Module):
    ...
    def forward_model(self, data):
        ...
        batch, edge_attr = data.batch, data.edge_attr
        features = []
        ...
        for layer in range(self.num_layers):
            ...
            if self.args.search_agg: # F2GNN variant
                x = self.gnn_layers[layer](tmp_input, edge_index, edge_attr, self.agg_weights[layer])
            else:
                x = self.gnn_layers[layer](tmp_input, edge_index, edge_attr)
            if self.args.batch_norm:
                x = self.bns[i](x)
