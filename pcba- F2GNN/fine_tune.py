...
def generate_args(arg_map):
    ...
    args.grad_clip = 5
    args.momentum = 0.9
    args.train_subset = args1.train_subset
    args.hidden_size = args1.hidden_size
    # args.num_workers = args1.num_workers
...
def run_fine_tune():
    ...
    hyper_space ={'model': 'f2gnn',
              'hidden_size': hp.choice('hidden_size', [args1.hidden_size]),
              'learning_rate': hp.uniform("lr", args1.lr),
              'weight_decay': hp.uniform("wr", 0.0001, 0.001),
              'optimizer': hp.choice('opt', ['adam']),
              'dropout': hp.choice('dropout', [args1.dropout]),
              'activation': hp.choice('act', ['elu'])
              }
    ...
    for ind, l in enumerate(lines):
        try:
            ...
            for i in range(args1.std_times):
                vali_mae, test_args = main(args1, run=i)
                t_mae = 0
                print('cal std: times:{}, valid_mae:{}, test_mae:{}'.format(i, vali_mae, t_mae))
                test_accs.append(t_mae)
