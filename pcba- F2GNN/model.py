...
class NaOp(nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr):
        # if self.with_linear:
        #     # return self.act(self._op(x, edge_index, edge_attr = edge_attr) + self.op_linear(x))
        #     return self._op(x, edge_index, edge_attr = edge_attr) + self.op_linear(x)
        # else:
        return self._op(x, edge_index, edge_attr = edge_attr)
            # return self.act(self._op(x, edge_index, edge_attr = edge_attr))
...
class NetworkGNN(nn.Module):
    ...
    def forward(self, data):
        ...
        for layer in range(self.num_layers):
            ...
            # fuse features
            # if not self.training and layer == 1:
            #     print(layer_input)
            tmp_input = F.relu(self.fuse_funcs[layer](layer_input))
