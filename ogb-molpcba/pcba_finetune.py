...
from pcba_model import NetworkGNN as Network
...
def get_args():
    ...
    parser.add_argument('--model_save_path', type=str, default='pcba_0206',
...
def main():
    ...
    genotype = args.arch
    # print(genotype)
    model = Network(genotype, aucm_criterion, args.hidden_size, 128, args.hidden_size,
    ...
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[40, 55],
                                                     gamma=0.0005)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        epoch, train_result, valid_result, test_result, optimizer.param_groups[0]['lr'], time.time() - start_time_local))
        start_time_local = time.time()
