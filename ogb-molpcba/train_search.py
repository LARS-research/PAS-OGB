...
parser.add_argument('--data', type=str, default='ogbg-molpcba', help='location of the data corpus')
...
parser.add_argument('--num_layers', type=int, default=18, help='num of layers of GNN method.')
...
def main():
    ...
    if args.data =='PPI' or args.data == 'ogbg-molhiv' or args.data == 'ogbg-molpcba':
    ...
    model = Network(args.data, criterion, num_features, 128, hidden_size, epsilon=args.epsilon,
                    args=args, with_conv_linear=args.with_conv_linear, num_layers=args.num_layers)
    print(data[0].task_type)
    ...
    for epoch in range(args.epochs):
        ...
        train_obj = train_graph(data, model, criterion, optimizer, arch_optimizer, lr, arch_lr)
        scheduler.step()
        ...
        if epoch % 1 == 0:
            logging.info('epoch=%s, train_loss=%f, valid_acc=%f, valid_loss=%f, test_acc=%f, test_loss=%f, explore_num=%s', epoch, train_obj, valid_acc, valid_obj, test_acc, test_obj, model.explore_num)
            print('epoch={}, train_loss={:.04f},valid_acc={:.04f}, valid_loss={:.04f},test_acc={:.04f},test_loss={:.04f}, explore_num={}'.
                  format(epoch, train_obj, valid_acc, valid_obj, test_acc, test_obj, model.explore_num))
...
def train_graph(data, model, criterion, model_optimizer, arch_optimizer, lr, arch_lr):
    ...
    for iter in range(train_iters):
        ...
        for i in range(args.w_update_epoch):
            ...
            # accuracy += output.max(1)[1].eq(train_data.y.unsqueeze(dim=1)[is_labeled].view(-1)).sum().item()
    ...
    return total_loss / len(data[4].dataset)
